...
export const updatePublication = async (id, payload) => {
  const response = await fetch(`${API_V1_URL}/update-publication/${id}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  })
...
export const removePublication = async (id) => {
  const response = await fetch(`${API_V1_URL}/delete-publication/${id}`, { method: 'DELETE' })
